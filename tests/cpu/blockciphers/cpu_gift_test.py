...
def test_gift(block_size, key_size, n_rounds, plaintexts, keys, ciphertexts):
    gift = Gift(block_size, key_size)
    blocks = np.array(gen_bits(plaintexts, block_size), gift.word_type)
    keys = np.array(gen_bits(keys, key_size), gift.word_type)
    # encryption test
    gift.encrypt(blocks, keys, 0, n_rounds)
    assert np.all(blocks == gen_bits(ciphertexts, block_size))
    # decryption test
    gift.decrypt(blocks, keys, n_rounds, n_rounds)
