...
from urca.blocks.present import Present
...
def test_present(text_size, key_size, n_rounds, plaintexts, keys, ciphertexts):
    ...
    texts = np.array(get_bits(plaintexts, text_size), present.word_type)
    keys = np.array(get_bits(keys, key_size), present.word_type)
    # encryption test
    present.encrypt(texts, keys, 0, n_rounds)
    assert np.all(texts == get_bits(ciphertexts, text_size))
    # decryption test
    present.decrypt(texts, keys, n_rounds, n_rounds)
    assert np.all(texts == get_bits(plaintexts, text_size))
